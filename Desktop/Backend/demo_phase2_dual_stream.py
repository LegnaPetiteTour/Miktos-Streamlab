...
class Phase2DemoRunner:
    ...
    async def initialize_components(self) -> bool:
        ...
        try:
            self.logger.info(
                "🚀 Initializing Phase 2 Dual-Stream Demo Components")
            ...
            try:
                ...
                self.logger.warning(
                    f"⚠️ OBS failed: {e}. Demo continues without OBS.")
            ...
            self.logger.info(
                "✅ Phase 2 dual-stream components initialized successfully")
            return True
    ...
    def _create_demo_config(self):
        ...
        self.logger.info(
            "📋 Creating Ottawa municipal dual-stream configuration")
        self.logger.info(f"   English Channel: {en_channel_id}")
        self.logger.info(f"   French Channel: {fr_channel_id}")
        self.logger.info(
            f"   SRT Backup: {
                srt_relay_url if srt_relay_url != 'srt://localhost:9999'
                else 'Demo mode'}")
    ...
    async def demonstrate_dual_streaming(self):
        ...
        try:
            self.logger.info(
                "🎯 Starting Phase 2 Dual-Language Streaming Demonstration")
            ...
            success = await self.dual_stream_manager.start_streaming(
                Language.ENGLISH)
            if success:
            ...
            if self.demo_config["test_language_switching"]:
                ...
                health = await self.dual_stream_manager.get_health_status()
                lang = health.get('active_language', 'unknown')
                self.logger.info(f"📊 Post-switch: Lang: {lang}")
            ...
            if self.demo_config["simulate_failures"]:
                self.logger.info(
                    "⚠️  Test 3: Simulating network failure...")
            ...
            self.logger.error(
                f"❌ Error during dual-streaming demonstration: {e}")
    ...
    async def _simulate_network_failure(self):
        ...
        self.logger.info(
            "   (Production: triggers automatic failover to SRT)")
        self.logger.info(
            "   Failover: loss >5% OR RTT >300ms OR frames >25")
        ...
        self.logger.info(
            "📺 Slate display: 'Technical Difficulties - Problèmes Techniques'")
    ...
    async def _demonstrate_health_monitoring(self):
        ...
        self.logger.info(
            f"   English Channel: {
                '✅ Configured' if channels.get(
                    'english',
                    {}).get('configured') else '❌ Not configured'}")
        self.logger.info(
            f"   French Channel: {
                '✅ Configured' if channels.get(
                    'french',
    ...
    async def demonstrate_municipal_compliance(self):
        ...
        self.logger.info(
            "   ✅ Enhanced thresholds (loss <3%, RTT <300ms)")
        self.logger.info(
            "   ✅ Faster failover detection (5 seconds vs. 10 seconds)")
