...
class CameraTile(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        self.studio_on_btn = ttk.Button(
            studio_frame,
            text="🌙 Studio ON",
            command=lambda: self.on_command(
                self.camera_id, "ENTER_STUDIO_MODE"
            )
        )
        ...
        self.studio_off_btn = ttk.Button(
            ...
            text="💡 Studio OFF",
            command=lambda: self.on_command(
                self.camera_id, "EXIT_STUDIO_MODE"
            )
        )
    ...
    def update_status(self, status: CameraStatus):
        ...
        self.status_label.config(
            text=state_icons.get(status.state, "⚪ OFFLINE")
        )

        # Battery
        battery_icon = "⚡" if status.battery_level > 80 else "🔋"
        self.battery_label.config(
            text=f"{battery_icon} {status.battery_level}%"
        )
        ...
        thermal_fg = (
            'red' if status.thermal_state in ['HOT', 'CRITICAL']
            else 'orange' if status.thermal_state == 'WARM'
            else 'black'
        )
        self.thermal_label.config(
            text=thermal_text,
            foreground=thermal_fg
        )
        ...
        self.network_label.config(
            text=network_icons.get(status.network_type, "📵 OFFLINE")
        )

        # Bitrate
        self.bitrate_label.config(
            text=f"📊 {status.current_bitrate_mbps:.1f} Mbps"
        )
        ...
        self.start_btn.config(
            state='normal' if is_stopped else 'disabled'
        )
...
class MultiCameraDirectorApp:
    ...
    def _create_ui(self) -> None:
        ...
        scrollbar = ttk.Scrollbar(
            tiles_container, orient="vertical", command=canvas.yview
        )
                ...
                async with websockets.connect(self.ws_uri) as websocket:
                    ...
                    async for msg in websocket:
                        ...
                        if isinstance(msg, bytes):
                            ...
                            message_str = bytes(msg).decode('utf-8')
                        else:
                            message_str = str(msg)
            ...
            if msg_type == 'camera_list':
                ...
                if camera_id and camera_id in self.camera_tiles:
                    status = CameraStatus(
                        ...
                        network_type=status_data.get(
                            'network_type', 'UNKNOWN'
                        ),
                        thermal_state=status_data.get('thermal_state', 'OK'),
                        current_bitrate_mbps=status_data.get(
    ...
    def check_thermal_alerts(self, camera_id: str, status: CameraStatus):
        ...
        if thermal == "HOT":
            self.log(
                f"🔥 WARNING: Camera {camera_id[:8]} is HOT - "
                "quality reduced"
            )
        elif thermal == "CRITICAL":
            self.log(
                f"☠️ CRITICAL: Camera {camera_id[:8]} thermal emergency!"
            )
            messagebox.showwarning(
                "Thermal Critical",
                f"Camera {camera_id[:8]} is at CRITICAL temperature!\n\n"
                "Consider switching to another camera or stopping this "
                "stream."
            )
        ...
        for cid, tile in self.camera_tiles.items():
            if cid == camera_id:
                ...
                if tile.status.state == "paused":
                    asyncio.create_task(
                        self._send_command_async(cid, "RESUME")
                    )
            else:
                ...
                if tile.status.state == "running":
                    asyncio.create_task(
                        self._send_command_async(cid, "PAUSE")
                    )
