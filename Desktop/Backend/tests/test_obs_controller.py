...
class TestSceneManagement:
    ...
    @pytest.mark.asyncio
    async def test_switch_scene_not_connected(
            self, controller):
...
class TestSourceManagement:
    ...
    async def test_show_slate(self, connected_controller):
        ...
        with patch.object(connected_controller, 'switch_scene',
                          return_value=True) as mock_switch:
            result = await connected_controller.show_slate(
...
class TestStreamingControl:
    ...
    async def test_start_streaming_already_active(
            self, connected_controller, mock_obs_response):
    ...
    @pytest.mark.asyncio
    async def test_get_stream_stats(
            self,
            connected_controller,
            mock_obs_response):
...
class TestHealthMonitoring:
    ...
    async def test_get_health_connected(
        ...
        connected_controller.ws.call.side_effect = [
            version_response, stats_response]
