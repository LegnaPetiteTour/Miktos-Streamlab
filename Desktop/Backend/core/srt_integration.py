...
class SRTStats:
    ...
    def get_health_score(self) -> float:
        ...
        if self.bandwidth_available_mbps > 0:
            utilization = self.bandwidth_used_mbps / self.bandwidth_available_mbps
...
class SRTConnection:
    ...
    async def connect(self) -> bool:
        ...
        """
        try:
            self.logger.info(f"Connecting to SRT destination: srt://{self.config.host}:{self.config.port}")
            ...
            if success:
                ...
                self._stats.peer_address = f"{self.config.host}:{self.config.port}"
                
                # Start monitoring
                await self._start_monitoring()
                
                self.logger.info(f"SRT connection established successfully")
                return True
            else:
    ...
    async def _connect_via_ffmpeg(self) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"Testing SRT connection with command: {' '.join(test_cmd)}")
            ...
            self.logger.error("FFmpeg not found. Please install FFmpeg with SRT support")
            return False
        except Exception as e:
            ...
            if self._ffmpeg_process:
                ...
                try:
                    await asyncio.wait_for(
                        asyncio.create_task(self._wait_for_process(self._ffmpeg_process)),
                        timeout=5.0
                    )
    ...
    async def start_streaming(self, input_source: str) -> bool:
        ...
        try:
            if not self._connected:
                self.logger.error("Cannot start streaming - not connected to SRT destination")
                return False
            ...
            self.logger.info(f"Starting SRT streaming: {' '.join(cmd[:-1])} [SRT_URL]")
            ...
            if self._ffmpeg_process.poll() is None:
                ...
                stderr_output = self._ffmpeg_process.stderr.read() if self._ffmpeg_process.stderr else "Unknown error"
                self.logger.error(f"SRT streaming failed to start: {stderr_output}")
                return False
    ...
    async def stop_streaming(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning("FFmpeg did not terminate gracefully, killing process")
                self._ffmpeg_process.kill()
    ...
    async def _update_stats(self):
        ...
        try:
            ...
            # Update basic stats (placeholder - would use actual SRT stats in production)
            self._stats.connected = True
            
            # In a real implementation, we would query libsrt for actual statistics
            # For now, simulate reasonable values
            if self._ffmpeg_process and self._ffmpeg_process.poll() is None:
                # Streaming is active
                self._stats.bitrate_mbps = 5.0  # Simulated bitrate
                self._stats.rtt_ms = 50.0 + (time.time() % 10) * 5  # Simulated RTT with variation
                self._stats.packet_loss_pct = max(0, (time.time() % 100) - 98) * 0.5  # Occasional loss
...
class SRTServer:
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"SRT server listening on srt://{self.host}:{self.port}")
            self._running = True
            return True
