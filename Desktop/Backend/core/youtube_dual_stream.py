...
Provides seamless bilingual streaming with automatic language detection and failover.
...
from .egress import RTMPDestination, EgressManager, EgressConfig, FailoverConfig
...
class DualStreamConfig:
    ...
    cross_language_failover: bool = True  # EN can failover to FR and vice versa
    ...
    professional_reliability: bool = True  # Enhanced reliability for professional use
...
class YouTubeDualStreamManager:
    ...
    Handles multiple language channels with intelligent failover between languages.
    Designed for global content creators requiring professional-grade reliability.
    """
    ...
    def _initialize_youtube_channels(self):
        ...
        try:
            ...
            en_platform_config = PlatformConfig(
                ...
                api_credentials={
                    k: v for k, v in {
                        ...
                        "access_token": self.config.english_channel.access_token,
            ...
            fr_platform_config = PlatformConfig(
                ...
                api_credentials={
                    k: v for k, v in {
                        ...
                        "access_token": self.config.french_channel.access_token,
            ...
            self.logger.info("YouTube channels initialized for dual-language streaming")
    ...
    def _initialize_egress_managers(self):
        ...
        try:
            ...
            primary_dest = (
                self.english_rtmp if self.config.primary_language == Language.ENGLISH 
            ...
            professional_failover = FailoverConfig(
                enabled=True,
                trigger_packet_loss_pct=3.0,  # More sensitive for professional use
            ...
            primary_config = EgressConfig(
                ...
                backup_destination={
                    ...
                    "name": srt_destination.name if srt_destination else "cross_language_backup",
                    "url": srt_destination.url if srt_destination else (
                        self.french_rtmp.url if primary_dest == self.english_rtmp 
    ...
    async def start_streaming(self, language: Optional[Language] = None) -> bool:
        """
        Start dual-language streaming
        
        Args:
            language: Specific language to stream (defaults to primary_language)
        ...
        try:
            ...
            self.logger.info(f"Starting dual-language streaming (primary: {target_language.value})")
            ...
            if success:
                self.streaming = True
                self.logger.info(f"Dual-language streaming started successfully")
    ...
    async def switch_language(self, target_language: Language) -> bool:
        ...
        try:
            if target_language == self.active_language:
                self.logger.info(f"Already streaming in {target_language.value}")
                return True
            
            self.logger.info(f"Switching streaming language from {self.active_language.value} to {target_language.value}")
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            status = {
                ...
                "channels": {
                    "english": {
                        ...
                        "healthy": False,  # Will be determined by egress manager
                    },
                    "french": {
            ...
            if self.primary_egress:
                ...
                status["failover_metrics"] = self.primary_egress.get_failover_metrics()
    ...
    async def _log_streaming_start(self):
        ...
        self.logger.info(f"Professional streaming session started: {log_entry}")
...
def create_dual_stream_config(
    ...
    Returns:
        DualStreamConfig: Configuration ready for professional dual-language streaming
    """
    ...
    return DualStreamConfig(
        english_channel=primary_channel,    # Note: These are kept as english/french for compatibility
        french_channel=secondary_channel,   # but can be any language pair
        primary_language=primary_language,
        failover_enabled=True,
        cross_language_failover=True,       # Primary can failover to secondary and vice versa
