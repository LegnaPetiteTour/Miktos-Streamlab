    ...
    try:
        ...
        try:
            ...
            if "already exists" in str(e).lower():
                print(f"⚠️  Scene already exists (OK)")
            else:
        ...
        print(f"   Base Resolution: {video_settings.base_width}x{video_settings.base_height}")
        print(f"   Output Resolution: {video_settings.output_width}x{video_settings.output_height}")
        ...
        print(f"✅ Scene management operational")
        print(f"\nNext steps:")
        print(f"  1. Start the Miktos Hub server")
        print(f"  2. Connect Android phones running the camera app")
        print(f"  3. Create multi-camera scenes via the API")
        ...
        print(f"\n❌ CONNECTION REFUSED")
        print(f"\n🔧 Troubleshooting:")
        print(f"   1. Make sure OBS Studio is running")
        print(f"   2. Open OBS Studio → Tools → WebSocket Server Settings")
        print(f"   3. Ensure 'Enable WebSocket server' is checked")
        print(f"   4. Verify port is set to {settings.obs.port}")
        print(f"   5. Click 'Apply' and 'OK'")
        print(f"   6. Restart OBS Studio if needed")
