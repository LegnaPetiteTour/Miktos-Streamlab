        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    conn_msg = f"✅ Connected to {addr[0]}:{addr[1]}"
                    print(f"{conn_msg} at {datetime.now()}")
            ...
            while self.running and self.client_socket:
                try:
                    ...
                    if data.startswith(b'\x00\x00\x00\x01'):
                        ...
                        # Log every ~50KB
                        if self.bytes_received % (50 * 1024) < len(data):
                            kb_total = self.bytes_received / 1024
                            msg = (
                                f"📡 Received {frame_type}: {len(data)} "
                                f"bytes (Total: {kb_total:.1f} KB)"
                            )
                            print(msg)
    ...
    def monitor_stream(self):
        ...
        while self.running and self.client_socket:
            ...
            if self.start_time:
                ...
                rate_kbps = (self.bytes_received * 8) / \
                    (elapsed * 1000) if elapsed > 0 else 0
                bytes_per_sec = (self.bytes_received -
                                 last_bytes) / 10 if elapsed > 0 else 0

                kb_received = self.bytes_received / 1024
                kb_per_sec = bytes_per_sec / 1024
                stats = (
                    f"📊 Stream Stats: {kb_received:.1f} KB received, "
                    f"Rate: {rate_kbps:.1f} Kbps, "
                    f"Current: {kb_per_sec:.1f} KB/s"
                )
                print(stats)
