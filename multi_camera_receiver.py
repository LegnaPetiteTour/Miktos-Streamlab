...
class MultiCameraReceiver:
    ...
    def __init__(self, base_port: int = 8554, num_cameras: int = 3):
        ...
        Args:
            base_port: Starting port (cameras will use base_port, base_port+1, etc.)
        ...
        print(f"📱 Configure phones to stream to:")
        print(f"   Server IP: <this-machine-ip>")
        print(f"   Ports: {base_port} - {base_port + num_cameras - 1}\n")
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    print(f"\n[{timestamp}] 🔗 {camera.camera_id} connected from {client_address}")
                    ...
                    # Start ffplay for preview if this is the first/active camera
    ...
    def _start_preview(self, camera_id: str) -> Optional[subprocess.Popen]:
        ...
        try:
            ...
            print(f"⚠️  Could not start preview for {camera_id}: {e}")
            return None
    ...
    def _receive_data(self, port: int):
        ...
        try:
            while self.running and client_socket:
                try:
                    ...
                    if not data:
                        ...
                        timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
                        print(f"\n[{timestamp}] ❌ {camera.camera_id} disconnected "
                    ...
                    if (port == self.active_camera_port and 
                        ...
                        try:
                            ...
                            print(f"⚠️  {camera.camera_id} preview window closed")
                    ...
                    if data.startswith(b'\x00\x00\x00\x01'):
                        ...
                        if current_fps < 2:
                            if camera.state != CameraState.PAUSED:
                                ...
                                timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
                                print(f"\n[{timestamp}] ⏸️  {camera.camera_id} "
                                      f"PAUSED (freeze frame mode - {current_fps:.1f} fps)")
                        else:
                            if camera.state == CameraState.PAUSED:
                                camera.state = CameraState.LIVE
                                timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
                                print(f"\n[{timestamp}] ▶️  {camera.camera_id} "
                                      f"RESUMED (live mode - {current_fps:.1f} fps)")
                        ...
                        if frame_count % 30 == 0 or current_fps < 2:
                            ...
                            mbps = (
                                (camera.bytes_received * 8) / (elapsed * 1_000_000)
                                if elapsed > 0 else 0
                            )
                            
                            timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
                            print(
                                f"🎬 [{timestamp}] {camera.camera_id} | "
                                f"{mode_indicator} | "
                    ...
                    timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
                    print(f"\n[{timestamp}] ❌ {camera.camera_id} receive error: {e}")
                    break
            ...
            timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
            print(f"\n[{timestamp}] 📊 {camera.camera_id} session ended")
            print(f"   Frames: {frame_count}")
            print(f"   Data: {camera.bytes_received / 1_000_000:.2f} MB")
    ...
    def _print_status(self):
        ...
        for port, camera in self.cameras.items():
            ...
            active_marker = "⭐" if port == self.active_camera_port else "  "
            
            if camera.state != CameraState.DISCONNECTED:
                uptime = time.time() - camera.start_time if camera.start_time > 0 else 0
                mbps = (
                ...
                print(
                    f"{active_marker} {state_icon} {camera.camera_id} (Port {port}): "
    ...
    def switch_active_camera(self, port: int):
        ...
        if port == self.active_camera_port:
            print(f"ℹ️  {self.cameras[port].camera_id} is already active")
            return
...
def main():
    ...
    receiver = MultiCameraReceiver(base_port=base_port, num_cameras=num_cameras)
    receiver.start()
