...
class SessionManager:
    ...
    def start_session(self, session_id: str) -> bool:
        ...
        if self._active_session_id and self._active_session_id != session_id:
            logger.error(f"Another session is active: {self._active_session_id}")
            return False
    ...
    def end_session(self, session_id: str) -> bool:
        ...
        self._log_event(session, "session_ended", {
    ...
    def remove_camera_from_session(
        ...
        for route in routes:
            self._stream_router.detach_camera_from_scene(camera_id, route.target_scene_id)
    ...
    def add_destination_to_session(
        ...
        logger.info(f"Added destination {destination.id} to session {session_id}")
        return True
    ...
    def remove_destination_from_session(
        ...
        session.destinations = [d for d in session.destinations if d.id != destination_id]
        ...
        self._log_event(session, "destination_removed", {"destination_id": destination_id})
        logger.info(f"Removed destination {destination_id} from session {session_id}")
        return True
