...
class RecordingConfig:
    ...
    # ISO settings
    record_cameras: Optional[List[str]] = None  # type: ignore
...
class RecordingInfo:
    ...
    iso_files: Optional[Dict[str, Path]] = None  # type: ignore
...
class RecordingService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Recording service initialized "
            f"(directory: {self._recordings_dir})"
        )
    ...
    async def start_recording(
        ...
        """
        logger.info(
            f"Starting recording for session {session_id}, "
        ...
        try:
            ...
            program_file = None
            if config.mode in [RecordingMode.PROGRAM_ONLY,
                               RecordingMode.PROGRAM_AND_ISO]:
            ...
            iso_files = {}
            if config.mode in [RecordingMode.ISO_ONLY,
                               RecordingMode.PROGRAM_AND_ISO]:
                cameras = (
                    config.record_cameras if config.record_cameras else []
                )

                for camera_id in cameras:
                    iso_file = (
                        session_dir /
                        f"iso_{camera_id}_{session_id}.mp4"
                    )
                    ...
                    logger.info(
                        f"ISO recording started for {camera_id}: "
                        f"{iso_file}"
                    )
            ...
            if config.enable_replay_buffer:
                ...
                logger.info(
                    f"Replay buffer enabled "
                    f"({config.replay_buffer_duration}s)"
                )
    ...
    async def stop_recording(self, session_id: str) -> RecordingInfo:
        ...
        try:
            ...
            for camera_id in info.iso_files.keys():
                ...
                logger.info(
                    f"ISO recording stopped for {camera_id}"
                )
    ...
    async def save_replay(
        ...
        """
        logger.info(
            f"Saving replay for session {session_id} (duration={duration}s)")

        if not await self._replay_buffer.is_active(session_id):
            raise RuntimeError(
                f"Replay buffer not active for session {session_id}")
    ...
    async def get_replay_buffer_status(
