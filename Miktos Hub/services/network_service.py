...
class BandwidthTestResult:
    ...
    def get_headroom_percent(self) -> float:
        ...
        if self.target_bitrate_kbps == 0:
            return 0.0
        return (
            (self.achieved_bitrate_kbps - self.target_bitrate_kbps) /
...
class PredictionResult:
    ...
    recommendations: Optional[List[str]] = None  # type: ignore
...
class NetworkService:
    ...
    Example:
        ...
        if result.is_sufficient:
            bitrate_mbps = result.achieved_bitrate_kbps / 1000
            print(f"Network ready: {bitrate_mbps:.1f} Mbps")
        else:
    ...
    async def test_bandwidth(
        ...
        """
        logger.info(
            f"Starting bandwidth test (target: "
            f"{target_bitrate / 1000:.1f} Mbps, duration: {duration}s)"
        )
        ...
        try:
            ...
            if is_sufficient:
                ...
                recommendation = f"Network ready for streaming ({
                    headroom:.0f}% headroom)"
            else:
                issues = []
            ...
            logger.info(
                f"Bandwidth test complete: {
                    achieved / 1000:.1f} Mbps achieved, "
    ...
    async def start_monitoring(self, camera_id: str) -> MonitoringSession:
        ...
        if camera_id in self._monitoring_sessions:
            logger.warning(
                f"Already monitoring camera {camera_id}, "
    ...
    async def predict_stability(
        ...
        """
        logger.info(
            f"Predicting network stability for camera {camera_id} "
            f"({duration_minutes} minutes ahead)"
        )
        ...
        if len(history) < 5:
            ...
            return PredictionResult(
                ...
                recommendations=[
                    "Not enough historical data for accurate prediction"],
            )
        ...
        # Calculate trends
        bitrate_trend = self._calculate_trend(
            [m.upload_bitrate_kbps for m in recent])
        rtt_trend = self._calculate_trend([m.rtt_ms for m in recent])
        loss_trend = self._calculate_trend(
            [m.packet_loss_percent for m in recent])
        ...
        if will_degrade:
            ...
            will_be_stable = current_quality in [
            ...
            if current_quality == NetworkQuality.FAIR:
                recommendations.append(
                    "Network stable but marginal - monitor closely")
        ...
        logger.info(
            f"Prediction: stable={will_be_stable}, "
            f"quality={predicted_quality.value}, "
            f"confidence={confidence:.1f}"
        )

        return result

    def _classify_network_quality(
            self, rtt_ms: float, loss_percent: float) -> NetworkQuality:
