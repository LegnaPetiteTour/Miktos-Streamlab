...
class QualityService:
    ...
    async def analyze_frame(self, frame_data: Any, camera_id: str) -> QualityAnalysis:
        """
    ...
    async def analyze_stream(
        ...
        """
        logger.info(f"Starting stream analysis for camera {camera_id} (duration={duration}s)")
    ...
    async def compare_cameras(
        ...
        if worst_analysis.exposure_score < 50:
            recommendations.append(
                f"Camera {worst_camera}: Adjust exposure (currently {worst_analysis.exposure_score:.0f}/100)"
            )
        
        if worst_analysis.focus_score < 50:
            recommendations.append(
                f"Camera {worst_camera}: Improve focus (currently {worst_analysis.focus_score:.0f}/100)"
            )
        ...
        if max(scores) - min(scores) > 30:
            recommendations.append(
                "Large quality variance between cameras - consider matching settings"
            )
        ...
        logger.info(
            f"Comparison complete - Best: {best_camera} ({analyses[best_camera].overall_score:.1f}), "
            f"Worst: {worst_camera} ({analyses[worst_camera].overall_score:.1f})"
        )
    ...
    def get_recommendations(self, analysis: QualityAnalysis) -> List[str]:
        ...
        for issue in critical:
            recommendations.append(f"CRITICAL - {issue.type}: {issue.recommendation}")
        
        for issue in high:
            recommendations.append(f"Important - {issue.type}: {issue.recommendation}")
        
        for issue in medium:
            recommendations.append(f"Suggested - {issue.type}: {issue.recommendation}")
        ...
        if analysis.overall_score >= 80 and not recommendations:
            ...
            recommendations.append("Quality is acceptable - minor improvements possible")
