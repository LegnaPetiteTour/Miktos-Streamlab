...
This service provides FFmpeg-based video editing capabilities for cutting clips,
resizing to different aspect ratios, adding captions, and exporting for social platforms.
"""
...
class ExportService:
    ...
    def __init__(self):
        ...
        if not self._ffmpeg_available:
            logger.warning("FFmpeg not found - export functionality will be limited")
        ...
        logger.info(f"Export service initialized (directory: {self._exports_dir})")
    ...
    async def resize_video(
        ...
        if output_file is None:
            ...
            output_file = self._exports_dir / f"resized_{aspect_name}_{timestamp}.mp4"
    ...
    async def add_captions(
        ...
        """
        logger.info(f"Adding captions: {caption_file.name} to {input_file.name}")
    ...
    async def resize_for_platform(
        ...
        if output_file is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_file = self._exports_dir / f"{platform_lower}_{timestamp}.mp4"
        ...
        try:
            ...
            logger.error(f"Failed to optimize for platform: {e}", exc_info=True)
            raise
