...
try:
    ...
    OBSStatus = None  # type: ignore
    StreamingStatus = None  # type: ignore
    OBS_AVAILABLE = False
...
class OBSEngineAdapter:
    ...
    def __init__(
        ...
        logger.info(
            f"OBS Engine Adapter initialized "
    ...
    async def create_scene(
        ...
        Args:
            name: Scene name
            layout_type: Layout type (converted to SceneLayout)
        ...
        try:
            ...
            # Convert layout_type string to SceneLayout enum
            try:
                layout = SceneLayout(layout_type)
            except ValueError:
                layout = SceneLayout.CUSTOM
    ...
    async def attach_camera(
        ...
        try:
            ...
            if camera.transport == TransportType.SRT:
                ...
                # For SRT, URL format: srt://host:port?streamid=...
                srt_url = (
                    camera.metadata.srt_url
                    if hasattr(camera.metadata, 'srt_url')
                    else camera.id
                )
                source_settings = {
                    "input": srt_url,
                ...
                # Note: This requires extending OBS controller to support
                # creating sources. For now, log what would be created.
                logger.info(
                    f"Would create SRT source '{source_name}' "
                ...
                logger.warning(
                    f"Unsupported transport type: {camera.transport}"
                )
                return False
    ...
    async def detach_camera(self, camera_id: str, scene_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Would remove source '{source_name}' "
                f"from scene '{scene_name}'"
            )

            # TODO: Implement source removal in obs_controller.py
            # success = await self._controller.remove_source(
