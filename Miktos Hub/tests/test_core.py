...
Tests the foundational services: DeviceRegistry, SessionManager, StreamRouter, EventBus
"""
import pytest
...
class TestDeviceRegistry:
    ...
    def test_register_duplicate_device_updates(self, device_registry, mock_camera):
    ...
    def test_thread_safety_concurrent_registration(self, device_registry, mock_cameras):
...
class TestStreamRouter:
    ...
    def test_list_all_routes(self, stream_router):
        ...
        routes = [
            Route(id=f"route-{i}", camera_id=f"camera-{i}", scene_id=f"scene-{i}")
            for i in range(5)
        ]
...
class TestSessionManager:
    ...
    def test_create_session_generates_id_if_not_provided(self, session_manager):
    ...
    def test_update_session_state(self, session_manager):
        ...
        session_manager.update_session_state("test-session", SessionState.READY)
        updated = session_manager.get_session("test-session")
        assert updated.state == SessionState.READY
    
    def test_add_camera_to_session(self, session_manager, device_registry, mock_camera):
    ...
    def test_add_nonexistent_camera_raises_error(self, session_manager):
        """Test adding a camera that doesn't exist"""
        session = session_manager.create_session(
            session_id="test-session",
            name="Test"
        )
    ...
    def test_remove_camera_from_session(self, session_manager, device_registry, mock_camera):
...
class TestEventBus:
    ...
    @pytest.mark.asyncio
    async def test_handler_exception_doesnt_break_other_handlers(self, event_bus):
