...
from models import CameraDevice, Scene, SceneLayout
...
class OBSOrchestrator:
    ...
    async def create_scene_for_camera(
        ...
        try:
            ...
            # Add camera source to scene
            source_pos = self._calculate_fullscreen_position()
            
            await self._obs.add_source(
                scene_id=obs_scene["id"],
                source_config={
                    "name": camera.label,
                    "type": "srt_input",
                    "url": camera.connection_url,
                    "position": source_pos.to_dict(),
                }
            )
    ...
    async def create_multi_camera_scene(
        ...
        # Generate scene name
        if not scene_name:
            camera_labels = ", ".join(c.label for c in cameras[:3])
            if len(cameras) > 3:
                camera_labels += f" +{len(cameras) - 3} more"
            scene_name = f"Multi-Cam: {camera_labels}"
        
        logger.info(
            f"Creating multi-camera scene: {scene_name} "
            f"({len(cameras)} cameras, layout={layout.value})"
        )
        ...
        try:
            ...
            # Calculate positions for each camera
            positions = self._calculate_layout_positions(layout, len(cameras))
            ...
            logger.error(f"Failed to create multi-camera scene: {e}", exc_info=True)
            return None
    ...
    def _calculate_layout_positions(
        ...
        if layout == SceneLayout.FULLSCREEN:
            ...
            positions.append(SourcePosition(0, 0, width, self._canvas_height))
            positions.append(SourcePosition(width, 0, width, self._canvas_height))
        
        elif layout == SceneLayout.SPLIT_VERTICAL:
            # Top and bottom
            height = self._canvas_height // 2
            positions.append(SourcePosition(0, 0, self._canvas_width, height))
            positions.append(SourcePosition(0, height, self._canvas_width, height))
            ...
            positions.append(SourcePosition(pip_x, pip_y, pip_width, pip_height))
    ...
    async def switch_scene(
        ...
        try:
            # Apply transition
            transition_id = transition.value if transition else "cut"
            
            await self._obs.transition(
    ...
    async def update_camera_layout(self, camera_ids: List[str]) -> Optional[Scene]:
        """
    ...
    def _find_scene_for_cameras(self, camera_ids: List[str]) -> Optional[Scene]:
        """Find an existing scene with the given cameras."""
        camera_set = set(camera_ids)
        
        for scene in self._scenes.values():
            if set(scene.camera_ids) == camera_set:
                return scene
