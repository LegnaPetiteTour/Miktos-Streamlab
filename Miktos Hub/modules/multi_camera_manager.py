...
class MultiCameraManager:
    ...
    def __init__(
        ...
        # Discovery settings
        self._mdns_service_name = config.camera.mdns_service_name or "_miktos._tcp.local."
        self._discovery_interval = config.camera.discovery_interval_seconds or 5.0
        self._health_check_interval = config.camera.health_check_interval_seconds or 10.0
    ...
    async def _discovery_loop(self) -> None:
        ...
        """
        logger.info(f"Discovery loop started (service: {self._mdns_service_name})")
            ...
            try:
                ...
                for camera in discovered:
                    if camera.id not in self._discovered:
                        logger.info(f"New camera discovered: {camera.label} ({camera.id})")
    ...
    async def _check_camera_at(self, ip: str, port: int) -> Optional[CameraDevice]:
        """
        ...
        try:
            ...
            camera = CameraDevice(
                ...
                connection_url=f"srt://{ip}:{device_info.get('srt_port', 8554)}",
                capabilities=device_info.get("capabilities", ["video", "audio"]),
                metadata={
                    "ip": ip,
                    "port": port,
        ...
        except (asyncio.TimeoutError, ConnectionRefusedError, json.JSONDecodeError):
    ...
    async def generate_pairing_code(
        ...
        logger.info(f"Generated pairing code: {code} (expires in {ttl_seconds}s)")
    ...
    async def pair_camera_manual(
        ...
        if pairing_code not in self._pairing_requests:
            logger.error(f"Invalid or expired pairing code: {pairing_code}")
            return False
        ...
        logger.info(f"Pairing camera manually: {camera.label} (code: {pairing_code})")
    ...
    async def _health_monitoring_loop(self, camera_id: str) -> None:
        ...
        """
        logger.debug(f"Health monitoring loop started for camera: {camera_id}")
        ...
        while True:
            try:
                ...
                try:
                    network_metrics = await self._network_service.get_metrics(camera_id)
                    
                    # Update camera health from network metrics
                    camera.health.network_quality = network_metrics.quality.value
                    camera.health.is_connected = network_metrics.is_connected
                    
                except Exception as e:
                    logger.error(f"Network check failed for {camera_id}: {e}")
                    camera.health.is_connected = False
                
                # Get battery/thermal from camera metadata (phone app reports this)
                camera.health.battery_percent = camera.metadata.get("battery_percent", 0)
                camera.health.temperature_celsius = camera.metadata.get("temperature_celsius", 0)
                ...
                if camera.health.overall_status == "critical":
                    logger.warning(f"Camera {camera.id} in critical state: {camera.health}")
                    
                    await self._event_bus.publish("camera_health_critical", {
                        "camera_id": camera.id,
                        "health": camera.health.to_dict(),
                        "timestamp": datetime.now().isoformat(),
                    })
                ...
                logger.error(f"Health monitoring error for {camera_id}: {e}", exc_info=True)
                await asyncio.sleep(self._health_check_interval)
    ...
    def add_discovery_callback(self, callback: Callable[[DiscoveryEvent], None]) -> None:
        """
