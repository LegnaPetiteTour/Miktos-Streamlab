...
try:
    from core.egress_v2 import MultiDestinationManager, StreamDestination
    from core.multi_destination_manager import DestinationHealth
    STREAMING_AVAILABLE = True
except ImportError as e:
    MultiDestinationManager = None
    StreamDestination = None
    DestinationHealth = None
...
class MultiPlatformStreaming:
    ...
    Example:
        ...
        await streaming.configure_destinations(
            session_id="show-001",
            destinations=[
                {"platform": "youtube", "stream_key": "xxx", "label": "YouTube EN"},
                {"platform": "youtube", "stream_key": "yyy", "label": "YouTube FR"},
        ...
        print(f"Healthy: {health.healthy_destinations}/{health.total_destinations}")
    ...
    def __init__(
        ...
        self._egress = MultiDestinationManager(
            ...
            health_check_interval=config.streaming.health_check_interval_seconds,
        )
    ...
    async def configure_destinations(
        ...
        """
        logger.info(f"Configuring {len(destinations)} destinations for session {session_id}")
        ...
        try:
            ...
            for dest_config in destinations:
                ...
                backup_enabled = dest_config.get("backup_enabled", True)
            ...
            logger.error(f"Failed to configure destinations: {e}", exc_info=True)
            return False
    ...
    def _get_platform_rtmp_url(self, platform: str, stream_key: str) -> str:
        ...
        return urls.get(platform.lower(), f"rtmp://custom-server/live/{stream_key}")
    ...
    async def start_stream(
        ...
        if not session.destinations:
            logger.error(f"No destinations configured for session: {session_id}")
            return False
        ...
        try:
            ...
            for destination in session.destinations:
                try:
                    ...
                    logger.error(f"Failed to start destination {destination.name}: {e}")
            ...
            if healthy_count == 0:
                ...
                logger.warning(f"Started with degraded status ({healthy_count}/{len(session.destinations)})")
            else:
                self._active_streams[session_id] = StreamStatus.LIVE
                logger.info(f"Stream started successfully ({healthy_count} destinations)")
        ...
        try:
            ...
            for destination in session.destinations:
                try:
                    ...
                    logger.error(f"Error stopping destination {destination.name}: {e}")
            
            # Remove from active streams
            del self._active_streams[session_id]
            
            logger.info(f"Stream stopped successfully")
    ...
    async def get_health(self, session_id: str) -> Optional[StreamHealth]:
        ...
        try:
            ...
            for destination in session.destinations:
                try:
                    health = await self._egress.get_destination_health(destination.id)
                    ...
                    logger.error(f"Failed to get health for {destination.id}: {e}")
                    failed_count += 1
    ...
    async def _health_monitoring_loop(self, session_id: str) -> None:
        ...
        logger.debug(f"Health monitoring loop started for session: {session_id}")
        ...
        while True:
            try:
                ...
                logger.error(f"Health monitoring error for {session_id}: {e}", exc_info=True)
                await asyncio.sleep(interval)
        
        logger.debug(f"Health monitoring loop stopped for session: {session_id}")
