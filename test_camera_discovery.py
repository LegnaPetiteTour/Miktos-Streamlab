...
def test_server_health():
    ...
    try:
        ...
        print(f"   cd '/Users/atorrella/Desktop/Miktos Streamlab/Miktos Hub'")
        print(f"   source ../.venv/bin/activate")
        print(f"   uvicorn api.server:create_app --factory --host 127.0.0.1 --port 8000")
        return False
...
def test_camera_health():
    ...
    for camera in cameras:
        ...
        try:
            response = requests.get(f"{BASE_URL}/api/cameras/{camera_id}/health", timeout=2)
            if response.status_code == 200:
                ...
                print(f"\n⚠️  {camera.get('name')}: Health check failed ({response.status_code})")
        except Exception as e:
            print(f"\n❌ {camera.get('name')}: {e}")
